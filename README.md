...
First look in `hls_action_csynth.rpt` file. There will be an important table, that provides information about resources used:
...
So please open `convert_csynth.rpt` file. There is a table that describes loop performance:
``
        * Loop:
        +-----------+---------+---------+----------+-----------+-----------+------+----------+
...
$ vivado_hls -f hls.tcl
``
Now checking `convert_csynth.rpt`  again, we see the performance changed: 
``
        * Loop:
        +----------+---------+---------+----------+-----------+-----------+------+----------+
        |          |  Latency (cycles) | Iteration|  Initiation Interval  | Trip |          |
...
Performance table in `convert_csynth.rpt` should be as following:
``
        * Loop:
        +----------+---------+---------+----------+-----------+-----------+------+----------+
        |          |  Latency (cycles) | Iteration|  Initiation Interval  | Trip |          |
